...
import (
	...
	"github.com/bytom/blockcenter/coin/btm"
	"github.com/bytom/blockcenter/protocol"
	"github.com/bytom/blockcenter/protocol/bytom"
	"github.com/bytom/blockcenter/service"
)

var bytomErrMap = map[string]error{
	"BTM712": service.ErrInputUTXONotFound,
}
...
func (c *Client) getRawBlock(req *getRawBlockReq) (protocol.WrapBlock, *bc.TransactionStatus, error) {
	...
	return bytom.NewWrapBlock(res.RawBlock), res.TransactionStatus, nil
}
...
func getAddressFromTxInput(tx *types.Tx, i int, netParam string) (string, error) {
	...
	case *bc.Spend:
		...
		return getAddressFromControlProgram(controlProgram, netParam)
	default:
		return "", nil
	}
}

func getAddressFromControlProgram(program []byte, netParam string) (string, error) {
	return btm.ScriptToAddress(program, btm.GetNetParams(netParam))
}
